...
<body>
	...
	<table style="text-align: center; margin: 0 auto; width: 1000px;">
		...
		<tr>
			<th>내용</th>
			<td colspan="5" class="left" style="height: 150px;">${fn: replace(vo.content, crlf, '<br>') }</td>
		</tr>
		<tr>
			<th>첨부파일</th>
			<td colspan="5" class="left">${vo.filename }<%@ taglib
				...
				<c:if test="${ !empty vo.filepath }">
					<a class="btn-fill-small"
						onclick="location='download.bo?id=${vo.id}&subject=${subject }'">다운로드</a>
				</c:if>
			</td>
		</tr>
	</table>
	...
	<c:if test="${login_info.userid eq vo.userid}">
		<a class="btn-fill"
			onclick="$('#detail').attr('action', 'modify.bo'); $('#detail').submit()">수정</a>
		<a  onclick="if( confirm('정말 삭제하시겠습니까?') ){ location='delete.bo?id=${vo.id}&subject=${subject }' }" class="btn-fill">삭제</a>
	</c:if>
...
function go_comment_regist(){
	if(${empty login_info } ){
		...
		location.href='logintest.me?subject=${subject}&id=${vo.id}';
		return;
	}
	if( $.trim($('#comment').val()) == '' ){
		alert("댓글을 입력하세요!");
	...
	$.ajax({
		type: 'post',
		data: { content: $('#comment').val(), pid: ${vo.id}, subject: '${subject}' },
...
function go_comment_list(){
	$.ajax({
// 		type: 'post',
		url: 'board/comment/${vo.id}/${subject}',
//  		url: 'list.no',
		success: function(result){
